...
| S03-D12 | **Notebook location and naming**: `notebooks/<stepNN>_<purpose>.ipynb` (e.g., `notebooks/step03_viz_demo.ipynb`). Notebooks assume execution from repo root; first code cell asserts `(Path.cwd() / "pyproject.toml").exists()` as a guard. First markdown cell describes purpose + prerequisite (`pip install -e ".[dev]"`). | Establishes the convention before notebooks accumulate. Numeric-prefix doc convention (000_*) is *not* used for notebooks — different namespace. |
| S03-D13 | **Notebook artifact output location**: `outputs/notebooks/<notebook_stem>/<run_id>/` where `run_id` = ISO `YYYYMMDDTHHMMSS`. Mirrors `RunConfig.run_folder()` style. `outputs/` is already gitignored ([D014](000_Architecture_Decisions.md)). | Keeps notebook artifacts grouped per-notebook + per-run, never in repo root. Reuses run_folder pattern so future tooling can scan both runs/ and notebooks/ uniformly. |
...
- Write `notebooks/step03_viz_demo.ipynb` (S03-D12) cells:
  ...
  2. **Code (guard)**: `from pathlib import Path; ROOT = Path.cwd(); assert (ROOT / "pyproject.toml").exists(), "Run from repo root"`.
