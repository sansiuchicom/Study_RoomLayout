...
- The program adapter (S04-D3) maps `ProgramRequest.floor_programs[level]`
  ...
  target-agnostic.
- Anchor handling — **only the donut-hole half** lands this Step:
  `subtract_anchors` (4.4) ports + is geometry-validated (hole excluded,
  regions sane). The `vertical_circulation` fixed-room re-insertion +
  full anchor-aware pipeline + anchor fixture move to **Step 07** (S04-D4
  revised — see §2): they need the polygon `LabeledRoomLayout` that
  Step 07 assembles, and anchors are the one non-Cell-port piece of this
  Step (no Cell reference, no 33-golden coverage), so they stay out of
  the faithful-port baseline.
...
| **4.15** | ~~`anchors.py` ② `vertical_circulation` fixed-room re-insertion + full anchor pipeline + fixture~~ **DEFERRED → Step 07** (scope split 2026-05-29, S04-D4 revised). Anchors are the only non-Cell-port piece of this Step (no Cell cross-check, 0 of 33 goldens); the fixed room is polygon-based and only becomes a real `LabeledRoom` at Step 07 labeling (Pipeline §3.8). The donut-hole half (`subtract_anchors`, 4.4) already landed as a tested forward helper. Numbering preserved. | n/a |
...
- **Anchor fixed-room re-insertion + full anchor-aware pipeline** (4.15)
  → deferred to **Step 07** (scope split 2026-05-29). Only the donut-hole
  `subtract_anchors` (4.4) lands in Step 04; the polygon fixed room is a
  Step 07 labeling concern, and anchors are the lone non-Cell-port piece
  (no cross-check / golden). Part of the Step 07 anchor/connectivity
  cluster — see §7.
...
| **7→4 role collapse correctness** (S04-D3) — `hub`→`public`-first mapping + `RoomSpec.name = SpaceUnitSpec.id` identity recovery; a broken id round-trip would mislabel output roles at Step 07. | Adapter unit tests (4.14) assert the id round-trips and that the recovered 7-class role matches the input `SpaceUnitSpec`. `vertical_circulation` exclusion is tested in the adapter (4.14); fixed-room re-insertion + the full-pipeline anchor fixture are deferred to Step 07 (4.15 scope split). |
...
**Carried to Step 07 (entry point + labeling) — the anchor / connectivity
cluster** (all deferred from Step 04, all converge where the region-based
result becomes the polygon `LabeledRoomLayout`):

- **Anchor fixed-room re-insertion** (4.15 deferred): wire
  `subtract_anchors` (4.4) into the per-floor `run()` loop (subtract →
  atomize → … → carve), then assemble `vertical_circulation` rooms from
  the anchor footprints as `LabeledRoom`s; `host_role=None` shafts stay
  forbidden holes. Add the first anchor fixture + full anchor-aware
  pipeline test here.
- **Corridor single connected component** (PHASE8 §11; xfail PoC in
  `test_stages_corridor.py`): post-Stage-2 bridge-carve, damage-guarded.
- **Access guarantee** (S04-D4): corridor ↔ `vertical_circulation` room
  connectivity.

These three are one body of work — corridor/anchor connectivity at the
output boundary — best done together once Step 07 produces real polygons.
