...
2. **`TargetRules` dataclass + 외부 JSON config + `rules_loader`** — Stage 02 게이트가 사용할 모든 도메인 default 값을 proto3 패키지 *내* `src/proto3/data/target_rules/apartment.json` 으로 분리 (importlib.resources 친화). `ApartmentAdapter(rules_path: Path)` required argument + `DEFAULT_APARTMENT_RULES_PATH` 상수 export. 외부 scan-to-BIM 파이프라인은 자기 json 만들어 path swap. Step 04 Def-11 결산.
...
5. **Stage 02 Domain Feasibility Gate 본격화** — total-area / min-dimension(bbox-level) / multi-floor placeholder 3개 게이트. `proto3.constraints.gates` 모듈 (4 pure functions, access-schema 포함하지만 검증 회로는 unit test 까지만 — 본격 인스턴스화 Step 09-10). `DomainGateFailure` 부모 + `AreaGateFailure` / `DimGateFailure` / `AccessSchemaFailure` 자식. R2 fixture 회로 작동 (review #2). Pipeline §9.10 Stage 02 outputs 항목 갱신.
...
7. **Fail-loud 정책 일관 적용** — `viz.palette.role_to_palette_key` 미지 role → ValueError (review #12), `viz.svg.render(...)` 의 unsupported kwarg (atoms/regions/spine) → ValueError (Step 07 본격 territory 진입 전 strict; 외부 review #11), `ApartmentAdapter.load_fixture` 가 fixture target_type 불일치 시 fail (외부 review #8).
...
| DoD-7 | `ApartmentAdapter(rules_path: Path)` — `rules_path` required argument + `DEFAULT_APARTMENT_RULES_PATH = Path(__file__).resolve().parent.parent / "data" / "target_rules" / "apartment.json"` 상수 export | unit test |
| DoD-8 | `ApartmentAdapter.load_fixture(path)` — fixture 의 `target_type` 이 `"apartment"` 가 아니면 ValueError (review #8) | unit test |
| DoD-9 | `stages/stage00_load.run` 의 `adapter is None` 분기에서 `ApartmentAdapter(DEFAULT_APARTMENT_RULES_PATH)` 사용 (Step 04 호환) | regression test |
...
| **S06-D5** | `ApartmentAdapter(rules_path: Path)` — **rules_path required argument**. `None` 허용 안 함. `proto3.target.apartment.DEFAULT_APARTMENT_RULES_PATH` 상수만 export. 한 가지 예외: `stage00_load.run(adapter=None)` 분기는 한 곳에서만 default path 사용 (DoD-9) | 사용자 가치관 (대화 2026-05-09): "default 자동 깔림 지양 — 점점 커지면 디버깅 어려움". 모든 호출 site 가 어떤 rules path 쓰는지 코드에 박힘. silent fallback 없음 (D004/D005 fail-loud 정신과 일관) |
| **S06-D6** | Failure 계층: `DomainGateFailure(Exception)` 부모 + `AreaGateFailure` / `DimGateFailure` / `AccessSchemaFailure` 자식. 모두 내부에 `FailureRecord` 보관 (S04-D11 패턴). Pipeline §9.10 Stage 02 outputs 항목도 동시 갱신 | R2 fixture metadata `expected_failure: "AreaGateFailure"` 와 일치. 게이트별/공통 catch 둘 다 가능. 기존 `ProgramInstantiationFailure` 는 Stage 01 cardinality / role-validity 전용 유지. Pipeline §9.10 갱신은 외부 review #7 |
| **S06-D7** | `SpaceUnitSpec.min_area_m2` 가 None 인 경우 default fill 시점 = **Stage 01 instantiation**. ProgramInstance 는 concrete 상태로 produce. fill 값 source = `TargetRules.default_min_area_m2[role]` (외부 JSON 에서 옴). Stage 01 은 추가로 `required` / `min_dimension_mm` / `preferred_area_m2` 도 보존하고, duplicate `name` / unknown `role` / type mismatch 시 ProgramInstantiationFailure (외부 review #4) | D004 ("ProgramInstance is the source of cardinality") 정신과 일관. Step 04 Stage 01 frame 은 dict→`{name, role}` 만 추출했음 (외부 review #4) — Step 06 본격화에서 모든 필드 보존 |
...
| **S06-D15** | `ApartmentAdapter.load_fixture(path)` — fixture `target_type != "apartment"` 일 때 ValueError (외부 review #8). 기존 `stage00_load` 의 target_consistency 는 RunConfig 있을 때만 작동했음 | adapter 가 자기 영역 fixture 만 받도록 자체 검사. RunConfig 없는 경로 (직접 adapter 호출) 에서도 mismatch 잡힘. fail-loud 일관 |
...
│   ├── apartment.py                   # ApartmentAdapter(rules_path) required + DEFAULT_APARTMENT_RULES_PATH + load_fixture target_type 검사 (S06-D5, D15)
...
| 4.3 | `src/proto3/data/target_rules/apartment.json` + README.md 신규 (S06-D2, D3, D4). `pyproject.toml` 에 package-data 추가 (DoD-5). `proto3.target.base.TargetRules` dataclass (S06-D9). `proto3.target.rules_loader.load_target_rules` (S06-D4). `proto3.target.apartment.ApartmentAdapter(rules_path)` required + `DEFAULT_APARTMENT_RULES_PATH` (S06-D5) + `load_fixture` target_type 검사 (S06-D15). `stage00_load.run` adapter=None 분기 갱신 (DoD-9). unit tests | `feat: TargetRules + apartment.json data package + adapter target check (S06-D4, D5, D9, D15, D17)` |
...
| Def-10 | **Target B/C/D/E rules json + adapter 구현** | apartment 외 typology 추가 시 동일 패턴 (`src/proto3/data/target_rules/{house,hotel,warehouse,office}.json` + 각 adapter) | 각 Target 진입 시 |
...
| R-S06-7 | `ApartmentAdapter(rules_path)` required 변경이 Step 04 호출 site 깨짐 | 영향 범위: `stages/stage00_load.run` 단일. 변경 = `ApartmentAdapter()` → `ApartmentAdapter(DEFAULT_APARTMENT_RULES_PATH)` 한 줄 (DoD-9) |
...
- proto3 코드 변경 없음 — 외부가 자기 json 만들어 `ApartmentAdapter(rules_path=external_path)` 주입.
