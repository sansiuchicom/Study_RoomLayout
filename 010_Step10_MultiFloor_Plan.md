...
first multi-floor target: **house** (D001 / Pipeline §5.2). It is the last piece
before the `Study_RoomLayout` output can be plugged into the downstream
**ResearchBIM synthetic-BIM** Stage 4.

**The key framing — most of multi-floor is already done.** A pre-planning spike
(throwaway) ran a 3-floor house — and a 3-floor house with a 2nd-floor courtyard
hole — through the existing `run()`: both returned `valid=True`, 0 failures, with
the shared stair re-inserted on every floor and rooms growing around the
courtyard. The reason: the geometry half (atomize → growth → carve → labeling) is
...
ResearchBIM's footprint passing lands); Step 10 uses **hand-built fixtures that
model the `Building` shape**.
...
1. house typology (10.2): data/target_rules/house.json (requires_single_floor=
   false; building-level min_cardinality; full default_min_area_m2 Role map),
   registered in run()'s _RULES_PATH_BY_TYPE. The 4-role model is confirmed to
   fit a residential house (spike-verified: hub=landing/living, private=bedroom,
   wet=kitchen/bath, service=utility, vc=stair).

2. Building-level cardinality (10.3): for a multi-floor typology, min_cardinality
   is checked over the SUM across floors (a house needs 1 kitchen building-wide,
   not per floor), not per-floor. Single-floor typologies (apartment) keep the
   exact per-floor behavior — byte-identical. Emits a ProgramInstantiationFailure
   with the building-level code.

3. Vertical-circulation continuity (10.4, S10-D6): the union of all vc anchors'
   floor_ranges must form ONE connected vertical stack covering every floor —
   else VERTICAL_CIRCULATION_DISCONTINUOUS. Trivially passes for one shared core;
   catches N partial cores that leave a floor unreachable. (Anchor-in-every-floor
   containment already exists — verified, not re-added.)

4. run() restructure (10.5, S10-D2): the per-floor body extracted to a _run_floor
   helper; run() = cross-floor PRE pass (anchor continuity + building cardinality)
   → per-floor loop (unchanged) → cross-floor POST pass (building completeness).
   Pure, never-crashes (proto3:D018) preserved. The single-floor apartment path is
   BYTE-IDENTICAL (33 goldens + corpus B unchanged).

5. Building-level post-validation (10.6): vertical-circulation reachability
   (every floor served) + building program completeness, as cross-floor checks
   that per-floor gates cannot express. Composed into failure_records.

6. Multi-floor fixtures + goldens (10.7): house fixtures modeling a ResearchBIM
   Building — a 3-floor house (shared stair) + a courtyard-floor variant (full
   footprint model, S10-D7) + a discontinuity failure-injection. Goldens
   (GEOS 3.14.1). The input is shaped to map 1:1 from Building/Storey (S10-D8).
...
| **S10-D2** | Orchestrator = **evolve `run()`**, not a new entry | `run()` already loops `for floor in shape.floors`, so D001's "wraps per-floor `run()`" is satisfied by `run()` *being* the wrapper. Extract the per-floor body to `_run_floor`; add a cross-floor PRE pass (continuity + building cardinality) and POST pass (completeness). Rejected: a separate `run_building()` entry (would duplicate the loop + split the contract). |
...
| **S10-D5** | Cardinality scope = **building-level for multi-floor** | apartment checks `min_cardinality` per floor; a house needs e.g. ≥1 kitchen *building-wide*. For a multi-floor typology, cardinality sums across floors; single-floor typologies keep per-floor (byte-identical). The cardinality gate becomes building-aware. Per-floor **area/dim** gates stay per-floor (those are genuinely per-room/floor). |
| **S10-D6** | Vertical-circulation **continuity** validated | The union of all vc anchors' `floor_range`s must form one connected vertical stack covering every floor (consecutive floors bridged by ≥1 stair/elevator) — else `VERTICAL_CIRCULATION_DISCONTINUOUS`. Motivated by **S10-D7'** N-anchor reality: trivial for one shared core, real for N partial cores. (Anchor-in-every-floor containment already exists — `_check_anchor_footprint_containment` loops the range; verified, reused.) |
...
Decisions expected to emerge *during* build (recorded as **S10-D11+** when they
land): the exact building-cardinality code + where the gate splits per-floor vs
building; the continuity algorithm (interval-union vs graph); whether `_run_floor`
returns a small per-floor result struct or appends in place; the house.json
cardinality/area values + provenance grade.
...
```text
src/room_layout/
  run.py                 # MODIFIED: _run_floor extraction + cross-floor PRE/POST passes (S10-D2)
  data/target_rules/
    house.json           # NEW: multi-floor typology (requires_single_floor=false, S10-D3)
  constraints/
    gates.py             # MODIFIED: building-level cardinality (S10-D5); single-floor unchanged
    multi_floor.py       # NEW (or in gates): vc continuity + building completeness (S10-D6, 10.6)
  schema/
    failure.py           # MODIFIED: VERTICAL_CIRCULATION_DISCONTINUOUS + building-cardinality codes
...
Module split (continuity/completeness in `gates.py` vs a new `multi_floor.py`)
...
| **10.2** | `house.json` typology (S10-D3) + register in `_RULES_PATH_BY_TYPE`; confirm 4-role fit | loads via `load_target_rules`; multi-floor gate no longer rejects house |
| **10.3** | Building-level cardinality (S10-D5) — building-aware gate; **apartment per-floor byte-identical** | building-cardinality unit tests; 33 apartment goldens unchanged |
| **10.4** | vc continuity (S10-D6) — anchor `floor_range` union covers all floors, connected; `VERTICAL_CIRCULATION_DISCONTINUOUS` | continuity tests incl. a gap-injection (N partial cores) |
| **10.5** | `run()` restructure (S10-D2) — `_run_floor` extraction + cross-floor PRE/POST passes; never-crashes preserved | **apartment goldens + corpus B byte-identical**; multi-floor house runs |
| **10.6** | Building post-validation (10.6) — vc reachability + building completeness, composed into `failure_records` | building-validation tests |
| **10.7** | Multi-floor fixtures + goldens (S10-D7/D8/D9) — 3-floor house + courtyard variant + discontinuity injection; Building-shaped | multi-floor run-goldens green; courtyard valid |
...
validation (vc continuity, building cardinality/completeness), the input model
