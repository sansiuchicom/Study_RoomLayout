...
coordinates, per-role area / aspect constraints. Nothing else — no
adjacency graph, no corridor placeholder, no explicit hub flag. Hub is
**implicit** (first `public` role room, see below).
...
### Hub designation (implicit)

The first `RoomSpec` with `role == "public"` is the **hub**. No explicit
fixture flag — implicit by role + listing order.

- K ≥ 3 cases: `space_1` is the hub.
- K = 2 cases (24, 27): no `public` room → hub invariant disabled.

Algorithm enforces **weak hub-connectivity** during growth: every
assigned room must remain path-connected to the hub in the region-graph
induced subgraph (see README § Phase 7).

### Role minimum areas

Per-role absolute minimum area. Hard gate — a room ending up below its
role's `min_area` is reported in diagnostics (not silently accepted).

| role | min (m²) | rationale |
|---|---:|---|
| public | 8.0 | 의미 있는 거실/식당 최소 |
| private | 4.0 | 침실 최소 (한국 표준 ~5㎡) |
| wet | 2.0 | 욕실 최소 |
| service | 3.0 | 주방·다용도실 최소 |

These are absolute values, not derived from footprint or K. All 33
cases share the same `role_min_areas` table (this iteration).

### Role aspect ranges

Per-role default `target_aspect_range` used by the algorithm when a
RoomSpec doesn't override its own. Following proto3 D005, the range is
a **hard gate**: a candidate region whose absorption would push the
room's `bbox_aspect` outside the range is rejected.

| role | default range | rationale |
|---|---|---|
...
runtime to the containing region via `Polygon.contains(Point)`.
Coordinates are chosen by hand to:
...
There is **no explicit "fill X% of footprint" target**. Algorithm stops
when every room is saturated — either no unassigned neighbor exists, or
every neighbor violates the aspect / hub gate. Resulting unassigned
regions are what spine-first will turn into corridor / access in a
later phase.

### Footprint area note

Footprint area below is approximate (hand-computed from `cases.py`
rect/disk parameters). The runtime should recompute it from the actual
`ShapeInput.parts` union and use that as authoritative.
...
Footprint: `rect(0, 0, 14, 10)`, area 140 m².
...
Footprint: `rect(0,0,8,10) + rect(8,0,14,7)`, area 122 m².
...
Footprint: `rect(0, 0, 16, 10)`, area 160 m².
...
Footprint: `rect(0,0,16,3.8) + rect(0,6.2,16,10) + rect(0,3.8,4,6.2)`, area ≈ 131 m².  
...
Footprint: three overlapping rects, area ≈ 122 m².
...
Footprint: `rect(0, 0, 10, 10)`, area 100 m².
...
Footprint: `rect(0, 0, 20, 6)`, area 120 m².
...
Footprint: `rect(0, 0, 6, 20)`, area 120 m².
...
Footprint: `rect(0,0,12,5) + rect(0,5,5,12)`, area 95 m².
...
Footprint: `rect(0,0,14,5) + rect(0,5,6,14)`, area 124 m².
...
Footprint: `rect(0,0,14,3) + rect(0,3,3,14)`, area 75 m².
...
Footprint: `rect(0,7,14,12) + rect(10,0,14,7)`, area 98 m².
...
Footprint: `rect(0,4,14,8) + rect(5,0,9,12)`, area 88 m².  
...
Footprint: `rect(0,4,14,7) + rect(6,0,9,12)`, area 69 m².
...
Footprint: `rect(0,0,14,5) + rect(5,5,9,12)`, area 98 m².
...
Footprint: `rect(0,0,14,10) hole(4.5,3,8.5,6.5)`, area 126 m².
...
Footprint: `rect(0,0,14,10) hole(3,3,11,7)`, area 108 m².  
...
Footprint: `rect(0,0,12,8)` rotated 30° around (6,4), area 96 m².  
...
Footprint: `rect(0,0,12,8)` rotated 60° around (6,4), area 96 m².
...
Footprint: 7자 rotated 45° around its union centroid (≈7.27, 7.59), area 88 m².

| name | role | seed_position |
|---|---|---|
| space_1 | public | (7.5, 8.0) |
| space_2 | private | (4.0, 8.0) |
| space_3 | wet | (10.5, 5.5) |

### Case 22 — Main + wing 25°

Footprint: `rect(0,0,12,8)` + rotated wing (5×4) translated to (9,7), rot 25°, area ≈ 116 m².
...
Footprint: `rect(0,0,12,8)` + 2 rotated wings, area ≈ 126 m².  
*`case_name` in code is `"Mirror wings +/-30°"` (ASCII, matches cases.py); ± here is for readability.*

| name | role | seed_position |
|---|---|---|
| space_1 | public | (4.0, 4.0) |
| space_2 | private | (8.0, 4.0) |
| space_3 | private | (-1.0, 8.0) |
| space_4 | private | (12.0, 9.0) |
| space_5 | wet | (4.0, 1.5) |

### Case 24 — 7자 angled (−25° + 0°)

Footprint: 2 rotated rects forming a small angled 7, area ≈ 48 m².  
*K=2 원룸 pattern: 침실 + 욕실. No public room → hub invariant disabled.*
...
Footprint: disk at origin r=6, area ≈ 113 m².  
...
Footprint: ellipse centered at origin, xfact=8, yfact=4, area ≈ 100 m².
...
Footprint: top half of disk centered at origin r=6, area ≈ 57 m².  
*K=2 원룸 pattern. No public room → hub invariant disabled.*
...
Footprint: `rect(0,0,4,14) + rect(4,0,13,4) + disk(4,4,4)`, area ≈ 105 m².
...
Footprint: `rect(0,0,5,12) + rect(5,0,14,3) + rect(5,5,14,8) + rect(5,10,14,12)`, area 132 m².
...
Footprint: `rect(0,8,14,12) + rect(11,0,14,12) + rect(0,0,11,4)`, area 112 m².
...
Footprint: `rect(0,0,14,3) + rect(0,3,2.2,12)`, area ≈ 62 m².  
...
Footprint: `rect(0,0,16,12) + rect(16,0,22,5) + rect(16,6,21,10)`, area 242 m².  
...
Footprint: `rect(0,0,12,10) hole(3,3,7,7) + rect(8,5,15,9)`, area ≈ 116 m².
...
5. Every role appearing in any RoomSpec has an entry in
   `role_min_areas` and `role_aspect_ranges`.
```
