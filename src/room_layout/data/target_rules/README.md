...
|---|---:|:---:|---|---|
| `private` (침실/방) | **7.0** | **B — narrow statute, used as proxy** | 주택법 시행령 (Housing Act Enforcement Decree) §10①1 라목: each bedroom in a 도시형생활주택/소형주택 unit (전용 ≥ 30 m²) must be ≥ 7 m²; MOLIT describes it as "최소한의 주거여건" (minimum living condition). | **Downgraded A→B after the 2026-06-02 consolidation review.** The 7 m² statute is scoped to 도시형생활주택/소형주택 bedrooms, **not** all apartment types — the Korean minimum-residential regime is *dwelling-level* (total unit area + bedroom count + facilities), with no general per-room area minimum. 7.0 is the only per-room Korean figure, used here as the most defensible **proxy** for an enclosed `private` room (not a universal apartment minimum). Cross-check: England NDSS single bedroom 7.5 m². See `docs/000_typology_gate_applicability.md` §5. |
...
| `wet` (욕실/화장실) | **2.5** | C — fixture-clearance estimate | A functioning 욕실 (washbasin + toilet + shower) is minimally ~1.5 m × 1.6 m ≈ 2.4 m², rounded to 2.5; cf. India NBC combined WC+bath ≥ 2.8 m², toilet-only ≥ 1.1 m². | Grade-C: derived from fixture clearances, no Korean statutory per-WC area. **Area is the wrong axis for accessibility/workplace**: US ADA governs toilet rooms by *geometry* (turning space, fixture clearance — not area; an accessible WC needs ~4.6 m² *of the right shape*), and US OSHA sets WC *count* by occupancy. So 2.5 m² is a **non-accessible, residential** barrier only; it does not generalize to hotel/office/warehouse. See `docs/000_typology_gate_applicability.md` §3. Verify before citing. |
