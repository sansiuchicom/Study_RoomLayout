"""Stage 2 detour shortcut routing for corridor carving — Phase 8 (§4.13).

Plan reference: ``004_Step04_AlgorithmCore_Plan.md`` §4.13 + S04-D1.
Faithful port of Cell ``corridor_stage2.py`` (imports swapped to
``room_layout.stages.*``). Iterative ratio-greedy detour shortcuts between
room pairs whose corridor-distance / map-distance ratio exceeds the threshold,
with hub-collapsed BFS hop counting + carve-damage simulation retry.
"""
...
def _bfs_hop_collapse_hub(
    ...
    Hub is a free-movement zone in the real walking sense — passing through 1 vs
    5 hub regions is the same "one space traversal". Collapsing makes distance
    reflect this. Corridor stays region-by-region (corridors *are* long walks)
    and so do other rooms.
...
def _corridor_distance_hop(
    ...
    """BFS hop count from ``entr_src`` to ``entr_dst`` restricted to ``passable``
    (corridor ∪ hub ∪ entr_src ∪ entr_dst — see §4.1), with hub collapsed to a
    single supernode.
    """
    return _bfs_hop_collapse_hub(
        entr_src,
        ...
        for rid in room_regions:
            for nbr in region_adj[rid]:
                if nbr not in room_regions:
                    ...
                    break
        return entrances
    hub_regions = room_region_ids.get(hub_idx, set()) if hub_idx is not None else set()
...
def _astar_shortcut(
    *,
    ...
    Hub/corridor regions outside the start/goal entrances are hard-blocked so the
    resulting path is forced to carve through other rooms — the whole point of a
    "detour shortcut".
    """
    ...
    def cost(rid: int) -> float:
        ...
        is_outline = on_footprint_edge.get(rid, False) or any(
...
def _stage2_detour_shortcut(
    ...
    Mutates ``room_region_ids`` and ``unassigned_set`` in place when a shortcut
    path is committed. Returns the new shortcut corridor set + per-iteration
    diagnostics. Disconnection is forbidden via the same simulation +
    minimal-cut retry as Stage 1.
    """
    ...
    for it in range(1, _STAGE2_MAX_OUTER_ITER + 1):
        ...
        for i in range(n_rooms):
            for j in range(i + 1, n_rooms):
                ...
                if not a_regs or not b_regs:
                    continue
                entr_a = _find_entrances(i, hub_idx, room_region_ids, all_corridor, region_adj)
                entr_b = _find_entrances(j, hub_idx, room_region_ids, all_corridor, region_adj)
                if not entr_a or not entr_b:
                    continue
                passable = all_corridor | hub_regions_set | entr_a | entr_b
                dc = _corridor_distance_hop(
                    entr_a, entr_b, passable, hub_regions=hub_regions_set, region_adj=region_adj
                )
                if dc is None:
                    continue
        ...
        all_corridor = base_corridor | shortcut_corridor
        entr_a = _find_entrances(a, hub_idx, room_region_ids, all_corridor, region_adj)
        entr_b = _find_entrances(b, hub_idx, room_region_ids, all_corridor, region_adj)

        forbidden: set[int] = set()
        path: list[int] | None = None
        attempts = 0
        excluded_for_simulation = (
            room_region_ids[a] | room_region_ids[b] | hub_regions_set | all_corridor
        )
