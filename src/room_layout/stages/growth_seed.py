"""Cell-aware seed placement for room partition growth — Phase 7 (Step 04 §4.9).

Plan reference: ``004_Step04_AlgorithmCore_Plan.md`` §4.9 + S04-D1 / S04-D7.

Faithful port of Cell ``growth_seed.py`` — the auto-placement algorithm
(``auto_place_seeds_by_cells``: hub → piece coverage → load-balanced FPS
extras). This is the **production** seed path (the new schema has no manual
seed_position; S04-D7), so it is covered by its own tests + auto goldens
(4.12), not the manual-seed 33-case goldens.

Imports swapped to ``room_layout.stages.*``; the ``shape`` parameter is renamed
``floor`` (``FloorShape``) per S03-D13 — stages are floor-scoped. Algorithm
unchanged.
"""
...
from room_layout.stages.growth_cells import vertex_cells_of_piece
...
def _enumerate_cells_with_regions(
    ...
    for territory in territories:
        ...
        for piece_idx, piece in enumerate(territory.pieces):
            ...
            for cell in cells:
                cell_regions = [
                    rid for rid in piece_region_ids if cell.covers(region_poly_by_id[rid].centroid)
                ]
                if cell_regions:
                    out.append(
                        {
...
def auto_place_seeds_by_cells(
    ...
    Phase B — Territory coverage: each surviving territory not yet covered by
              hub gets 1 seed in its BIGGEST cell. Sorted by territory total
              area DESC. Stops at K.
    ...
    region_poly_by_id = {r.region_id: _to_shapely(r.shape) for r in region_graph.regions}
    ...
    if has_public:
        ...
        for idx, cell in enumerate(all_cells):
            if hub.region_id in cell["regions"]:
                ...
                piece_seed_count[(cell["territory"].part_id, cell["piece_idx"])] += 1
                break
    ...
    for pkey in uncovered_pkeys:
        ...
        for biggest_idx in piece_cells_idx:
            ...
            candidates = [regions_by_id[rid] for rid in cell["regions"] if rid not in used]
    ...
    while len(seeds) < K:
        ...
        for pkey, t_cells in cells_by_piece.items():
            ...
            if not non_hub:
                continue
            if any(any(rid not in used for rid in all_cells[i]["regions"]) for i in non_hub):
        ...
        if candidate_pkeys:
            ...
            target_cells = [
                i
                for i in cells_by_piece[target_pkey]
                if i != hub_cell_idx and any(rid not in used for rid in all_cells[i]["regions"])
            ]
            target_cell_idx = max(
                target_cells,
                key=lambda i: (
        ...
        candidate_regions = [regions_by_id[rid] for rid in cell["regions"] if rid not in used]
        if not candidate_regions:
            break

        existing_in_cell = [
            seed.region for seed in seeds if seed.region.region_id in cell["regions"]
        ]
        if not existing_in_cell:
            ...
            existing_centroids = [region_poly_by_id[s.region_id].centroid for s in existing_in_cell]
            picked = max(
                candidate_regions,
                key=lambda r: (
                    _min_centroid_dist(region_poly_by_id[r.region_id].centroid, existing_centroids),
                    region_area(r),
                    -r.region_id,
                ),
            )
...
def _min_centroid_dist(centroid: sg.Point, existing_centroids: list[sg.Point]) -> float:
    """Min Euclidean distance from ``centroid`` to any existing seed centroid."""
    return min(hypot(centroid.x - ec.x, centroid.y - ec.y) for ec in existing_centroids)
...
def _farthest_or_centrality(
    ...
    existing_centroids = [region_poly_by_id[sid].centroid for sid in seed_region_ids]

    def key(r):
        rc = region_poly_by_id[r.region_id].centroid
        min_hop = min(bfs_cache[sid].get(r.region_id, _INF_HOP) for sid in seed_region_ids)
        min_euc = min(hypot(rc.x - ec.x, rc.y - ec.y) for ec in existing_centroids)
